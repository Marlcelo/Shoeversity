...
if($total_records > 0) {
	...
	$output .= '<div class="alert alert-info alert-dismissible fade in" style="margin-top: -20px; margin-bottom: 200px">';
...
while($row = mysqli_fetch_assoc($result)) {
	...
	$output .= '<span class="thumbnail" style="min-height: 0px;  min-height: 460px" onclick="location.href='."'../../views/admin/view_product.php?pid=".$row['uid']."&token=".$token."';".'">';
	$output .='<!-- DELETE PRODUCT -->
                        <div class="col-md-12 col-sm-1">
                            <a href="dashboard.php?delete='.$row['uid'].'&token='.$token.'"><button class="btn btn-md btn-info pull-right" ><i class="glyphicon glyphicon-remove"></i></button></a>
                        </div> ';
